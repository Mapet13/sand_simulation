...
mod sand_simulator {
    ...
    impl Simulator {
        ...
        fn update_movable_particle(
            &mut self,
            id: usize,
            update_particle_fn: &mut dyn FnMut(
                &mut Simulator,
                usize,
                usize,
                Vec2<usize>,
        ...
        fn create_particles(&mut self, pos: Vec2<usize>) {
            match self.current_spawning_field {
                ...
                Some(FieldState::Water(_)) | Some(FieldState::Sand(_)) => {
                    self.create_particle_with_rules(
                        pos,
                        Simulator::falling_particle_creation_rules,
                    );
                }
                _ => {}
            }
                ...
                match event {
                    ...
                    Event::MouseButtonDown {
                        ...
                        match mouse_btn {
                            //todo: refactor here
                            MouseButton::Left => {
                                self.simulator.set_creation(true, FieldState::Sand(0))
                            }
                            MouseButton::Right => {
                    ...
                    Event::MouseButtonUp { mouse_btn, .. } => match mouse_btn {
                        MouseButton::Left => {
                            self.simulator.set_creation(false, FieldState::Sand(0))
                        }
                        MouseButton::Right => {
                            self.simulator.set_creation(false, FieldState::Water(0))
                        }
                        MouseButton::Middle => {
                            self.simulator.set_creation(false, FieldState::Wood(0))
                        }
                        _ => {}
                    },
